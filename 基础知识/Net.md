...
## **TIME-WAIT**的时间为什么是2\*MSL，能不能是小于2\*MSL的值？
...
## TCP保活机制
> 假如有这样的场景，客户端和服务端已经建立了TCP连接，可是客户端**主机突然奔溃了**（是主机，而不是进程），如果没有相应的措施，那么服务器进程可能一直等待客户端发送数据，造成服务端资源浪费。TCP使用保活计时器来处理这种情况。每次服务端收到客户端的报文后，服务端会重新设置保活计时器，通常设置为两小时；如果两小时之内服务端都没有收到来自客户端的消息，服务器就发送一个探测报文段，以后每隔75秒发送一次；连续发送10次后客户仍然没有回应，则服务端认为客户端发生故障，主动关闭连接。
...
> * 滑动窗口第二个作用就是用于流量控制。由于发送端和接收端处理数据的速度不同，当接收端接收数据的速度较慢时，可以通过TCP头部中的窗口字段来通知对方自己最大可以接收的数据大小，这样就可以防止接收端的接收缓冲区满，而导致数据丢弃。
...
>       * 拥塞窗口的单位是报文段的个数，在刚开始发送数据时，将拥塞窗口大小设置为一，其数据量为一个MSS大小。每经过一个轮次的发送，就将拥塞窗口加倍。像这样的拥塞窗口增长速度是指数级的。为了防止拥塞窗口增长的过大，引入一个阈值。当拥塞窗口小于这个指时，使用慢开始算法。当大于这个值时，使用拥塞避免算法。每次当网络发生拥塞时（即未能按时接受确认报文，即认为网络发生了拥塞），将阈值设置为当前拥塞窗口的一半，并且将拥塞窗口设置为1，从新启动慢开始算法。
>   * 拥塞避免算法
>       * 拥塞避免算法的思想是为了让拥塞窗口缓慢增长，每经过一个轮次的发送，将拥塞窗口加一，这样拥塞窗口的增长速度是线性级别的。拥塞避免算法只是为了控制拥塞窗口增长的速度，这样可以降低网络出现拥塞的概率，而并不是真的可以避免拥塞。
>   * 快重传和快恢复
>       * 快重传要求当接收方收到一个乱序报文，立即发送重复确认（即再次发送确认应答，确认序列号与上一次一样），以此来告诉发送方有报文段未到达接收端。快重传规定当发送方连续接收到三次重复确认报文，就立马重传对方尚未确认的报文，而不需要等待超时重传的计时器到期。这样可以尽早发送未确认的报文。
...
## TCP延时应答和捎带应答
> * 延时应答
>   > 当接收端接收数据时，如果立即发送确认应答，此时应用程序可能还没来得及将数据取走，所以确认应答中的窗口设置可能比较小；如果只需要等待很少的时间，应用程序就可以取走数据，那么将增大确认应答的窗口，从而增大传输效率。所以接收端可以等待一段时间，再返回确认应答，这就是延时应答。一般等待时间不超过200ms，并且一般最多可以隔两个包必须返回应答。
> * 捎带应答
...
>   > * UDP是**无序的**，而TCP是有序。消息在传输过程中可能会乱序，后发送的消息可能会先到达，TCP会对其进行重排序，UDP不会。
...
## 从输入URL到网页呈现的过程
> * **DNS域名解析**：DNS解析是一个递归查询的过程。例如，我们需要解析”www.google.com”时，会经历以下步骤：
...
> * **建立TCP连接**：一般在web服务过程中，使用的都是**HTTP**协议，所以需要先建立TCP连接：
>   > * 第一次握手：建立连接时，客户端向服务端发送请求报文
>   > * 第二次握手：服务器收到请求报文后，如同意连接，则向客户端发送确认报文
>   > * 第三次握手，客户端收到服务器的确认后，再次向服务器给出确认报文，完成连接。
> * **浏览器发送HTTP请求**：浏览器构建HTTP请求报文，并发送给服务器。
> * **服务器处理HTTP请求**：服务器接收到请求报文后，对其处理，并生成HTTP响应报文，返回给浏览器。
> * **浏览器页面显示**：浏览器接收到服务器发送过来的响应报文后，对其进行解析并显示。
> * 最后如果需要，可能要进行**TCP断开链接**的过程。
